...
import { FaHome, FaCheck, FaTimes, FaSpinner } from "react-icons/fa";

const AdminHomestaysPage = () => {
  ...
  const loadRequests = async () => {
    ...
    try {
      ...
      // Filter only CREATE requests
      const createRequests = Array.isArray(data) ? data.filter((r) => r.type === "CREATE") : [];
  ...
  return (
    <div className="admin-homestays-page">
      <div className="page-header">
        <h1>Yêu cầu Đăng ký Mới</h1>
        <p>Duyệt các yêu cầu đăng ký homestay mới từ chủ nhà</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p>Hiện tại không có yêu cầu homestay nào đang chờ duyệt</p>
        </div>
      ) : (
            ...
            <div key={request.id} className="request-card">
              <div className="request-header">
                ...
                <span className={`type-badge ${request.type.toLowerCase()}`}>
                  {request.type === "CREATE" ? "Tạo mới" : "Cập nhật"}
                </span>
              </div>
